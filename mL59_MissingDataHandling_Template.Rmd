...
```{r}
# code here
sum(complete.cases(credit_approval)) # this just gives numbers
...
```{r}
credit_approval_na_omitted <- na.omit(object = credit_approval)

# OR THIS - using pipe operator
credit_approval_na_omitted <- credit_approval %>% 
  na.omit()
```

The advantage is its simplicity, but it comes with **several drawbacks**.

- For small datasets you can't afford to lose much data. In this case you would lose `r (1-nrow(credit_approval_removed_NAs)/nrow(credit_approval))*100` % of all data (~5%).
...
```{r}
# an error happens here that I don't understand, and it happens in the solution code too
credit_approval_missForest <- missForest(credit_approval)
```
...
```{r}
mice_model <- mice(data = credit_approval, m = 10, method = 'pmm')
credit_approval_mice <- mice_model$data
```
...
credit_all_models <- rbind(
  credit_approval_na_omitted %>% add_column(method = "Removed NA"),
