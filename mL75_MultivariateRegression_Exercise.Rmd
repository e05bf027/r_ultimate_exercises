...
```{r}
cor_airfoil_noise <- cor(airfoil_noise)
corrplot.mixed(cor_airfoil_noise)
```

Which two variables correlate most to target variable?

```{r}
# ss_displacement_thickness and frequency
cor_airfoil_noise[1, c(1, 5)]
...
```{r}
airfoil_model_1 <- lm(data = airfoil_noise, 
                      formula = 'spl ~ .')
```
...
```{r}
# create predictions
airfoil_noise$spl_pred <- predict(object = airfoil_model_1, 
                                  newdata = airfoil_noise)

# perform model performance

postResample(pred = airfoil_noise$spl_pred, 
...
```{r}
airfoil_model_2 <- lm(data = airfoil_noise,
...
```{r}
# create predictions
airfoil_noise$spl_pred <- predict(object = airfoil_model_2, 
                                  newdata = airfoil_noise)
...
```{r}
# No! it decreased! Look at the decrease inb R^2 values!
```
