...
The last element is the element whose index equals the length of the vector. The length of the vector can be determined with length().
...
```{r}
fibCalc <- function(element_nr) {
  fibonacci <- c(1, 1)
  for (i in 3:element_nr) {
    next_element <- (fibonacci[length(fibonacci)]) + (fibonacci[length(fibonacci) - 1])
    fibonacci <- c(fibonacci, next_element)
  }
  fibonacci # R assumes last element is returned, so we don't need to return(fibonacci)
}
```
...
fibCalcV2 <- function(element_nr) {
  ...
  if (element_nr >= 3) {
    ...
    fibonacci <- 'element must be a positive integer'
    
  } else {
    fibonacci <- fibonacci[element_nr]
