...
We will analyse 'Hard Times' by Dickens.
...
Luckily this book is available for download. The link is stored in variable "url" and the text is downloaded with _readLines()_ and save in "text_hardTimes".
```{r import, cache=TRUE}
url <- "https://www.gutenberg.org/files/786/786-0.txt"
text_hardTimes <- readLines(con = url)   # con is the connection the function reads from
```
...
view(text_hardTimes)   # we can see the text begins on line 46
text_hardTimes_filt <- text_hardTimes[158:length(text_hardTimes)]
text_hardTimes_filt <- text_hardTimes_filt[1:(length(text_hardTimes_filt)-370)] #ends 7 lines earlier
tail(text_hardTimes_filt)   # confirms we are at the end
```
