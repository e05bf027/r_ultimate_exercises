...
author: "David Hannon (adaptation of Bert Gollnick)"
output: 
  html_document:
    highlight: tango
    toc: true
...
For analysis we need the _dplyr_ package. _ggplot2_ is used for visualisation. We can use tidyverse for this.
...
tbl_df(building)
```
There is a column "date" and a column "time". We need to convert them to a "POSIX" object.
...
# lubridate - tgis adds the new columns and fills it with 'lubridated' data
...
My understanding is that it is best to use POSIXct as the list structure of a date in POSIXlt can be troublesome and POSIXct takes up quite a bit less memory. `Lubridate` takes care of a lot of unclassing issues in the background.

```{r}
# classical approach
building$hour <- unclass(as.POSIXlt(building$timestamp))$hour

# lubridate (much simpler)])
...
hour_flow <- building %>% 
	group_by(hour, flow) %>% 
  summarise(count = sum(count))   # 'count' counts observations by group
