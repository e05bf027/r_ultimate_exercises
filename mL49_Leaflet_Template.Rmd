...
lf <- leaflet() %>% 
  ...
  addCircles(lng = deaths$long, # command literally adds circles to this map object
		   lat = deaths$lat, # this and the above give the center of this/these circles
		   radius = deaths$count*2, #  size of circle radius
		   stroke = F, # if the circle should have a stroke at its edge
		   color = "red", # what colour the circle should be 
		   fillOpacity = 0.6, # opacity of the fill colour inside the object
		   popup= paste("Deaths: ", deaths$count)) %>% # the text that should popup in the circle
  
  addCircles(lng=pumps$long, # as above, but for pumps
		   lat=pumps$lat, 
		   radius = 2, 
		   color = "green",
		   fillOpacity = 1,
