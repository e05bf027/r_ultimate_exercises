...
If you want to transform data from wide to tidy you can use functions from package **tidyr**. The relevant function is *pivot_longer()*. You need to provide some parameters
...
school_wide <- as_tibble(school_wide) # make sure it's a tibble!

school_long <- pivot_longer(data = school_wide, 
             cols = !name, 
             names_to =  'subject',
             values_to = 'grade')
...
The function with the opposite effect is *pivot_wider()*. You need to pass the exact same parameters:
...
school_tidy <- school_long %>% 
  group_by(subject) %>% 
  mutate(mean_grade_subject = mean(grade))
