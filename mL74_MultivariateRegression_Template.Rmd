...
```{r}
pairs(df[, 8:12])
# the result is not easy to remember how to code, and is not easy to read
```
...
```{r}
cor_df <- cor(df)
corrplot.mixed(corr = cor_df)
# corrplot.mixed gives graphical AND numeric representation of the correlation
```

This shows in the upper right triangle correlations as circles. Size corresponds to absolute value of correlation. We are looking for large circles, because they indicate high absolute correlations. The colors correspond to positive or negative correlations. Positive correlation between X and Y means, that an increase of X leads to an increase in Y. A negative correlation between X and Y means, that an increase of X leads to a decrease in Y.

Remember, for regression modelling we are assuming the variables are independent of each other, so verifying things in this way lets us explore this issue.
...
model <- lm(data = df, 
            formula = 'quality ~ .')
# the dot operator ('.') tells R to refer to the entire dataset
```
...
Predicted values and true values are visualised as correlation plot. A linear regression line is drawn for reference. Also a black line is drawn as reference for a perfect regression, in which predicted values and actual values are identical. 
...
The below plot allows us to assume the errors are normally distributed.
...
For more information about checkin assumptions for these types of things in R, see [here](https://www.youtube.com/watch?v=eTZ4VUZHzxw).
